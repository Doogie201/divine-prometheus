...
const Icon = ({ name, className }: { name: string; className?: string }) => {
  const icons: { [key: string]: JSX.Element } = {
    ...
    brain: <path d="M9.5 2A2.5 2.5 0 0 1 12 4.5v15a2.5 2.5 0 0 1-4.94.44c-.47-1.65-2.26-3.33-4.12-3.33-1.41 0-2.44 1.48-2.44 3.33A2.5 2.5 0 0 1 5.44 22H18.5a2.5 2.5 0 0 0 2.5-2.5v-15a2.5 2.5 0 0 0-2.5-2.5h-9Z" />, // For 'GPT-Rewrite'
    rocket: <path d="M4.5 16.5c-1.5 1.26-2 5-2 5s3.74-.5 5-2c.71-.84.7-2.3.05-3.05-.64-.75-2.19-.75-3.05-.05Z" />, // For 'Launch'
    alert: <path d="m21.73 18-8-14a2 2 0 0 0-3.46 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z" />, // For warnings
...
const ClarityGauge = ({ score }: { score: number }) => {
  ...
  const colorClass =
    score > 80 ? 'text-[var(--color-glow-success)]' :
    score > 50 ? 'text-[var(--color-glow-warning)]' :
  ...
  return (
    ...
    <div className="relative flex items-center justify-center" style={{ width: size, height: size }}>
      ...
      <span className={`font-mono text-3xl font-bold tracking-tighter ${colorClass}`}>{score}</span>
    </div>
  );
};

const AnalysisPill = ({ icon, text, type }: { icon: string; text: string; type: 'success' | 'warning' | 'danger' }) => {
  // Mapping types to the new glow color system
  const typeClasses = {
    success: 'text-[var(--color-glow-success)] border-[var(--color-glow-success)]/20 bg-[var(--color-glow-success)]/10',
    warning: 'text-[var(--color-glow-warning)] border-[var(--color-glow-warning)]/20 bg-[var(--color-glow-warning)]/10',
    danger: 'text-[var(--color-glow-danger)] border-[var(--color-glow-danger)]/20 bg-[var(--color-glow-danger)]/10',
  };

  return (
    // Component Module: Analysis Feedback Pill
    // Glassmorphism applied: frosted, layered UI with soft glow edges.
    <div className={`flex shrink-0 items-center gap-2 rounded-full border px-3 py-1.5 text-sm font-medium transition-all duration-300 hover:bg-white/10 ${typeClasses[type]}`}>
...
export default function PromptWorkbench({ onClose }: WorkbenchProps) {
  ...
  const [vault, setVault] = useState<VaultEntry[]>(() => {
    try {
      ...
      return (JSON.parse(localStorage.getItem('promptVault') || '[]') as VaultEntry[])
        .filter(v => v?.meta); 
    } catch {
      return [];
    }
    ...
    () =>
      debounce((text: string) => {
        ...
        if (match && text.endsWith(match[0])) {
          setSuggestion(text.slice(0, -match[0].length) + `specify the ${match[0]}`);
        } else {
          setSuggestion('');
        }
  ...
  const handleEnhance = async () => {
    if (!analysis) return;
    const result = await simulate('Enhance Prompt', async () => enhancePrompt(analysis), { stub: { success: { metaPrompt: 'stub', reasoning: [] }, failure: new Error('fail') } });
    setEnhanced(result);
    ...
    addToast({ type: 'success', title: 'Enhanced', message: 'Prompt saved to vault.' });
  ...
  const rewriteWithGPT = async () => {
    if (!raw.trim()) return;
    addToast({ type: 'info', title: 'GPT-Rewrite', message: 'Contacting LLM...' });
    try {
      const res = await fetch('/api/rewriter', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ prompt: raw }) });
      const { rewritten } = await res.json();
      setRaw(rewritten);
      addToast({ type: 'success', title: 'GPT-Rewrite Complete', message: 'Prompt updated.' });
    } catch {
      addToast({ type: 'error', title: 'GPT-Rewrite Failed', message: 'Endpoint unreachable.' });
    }
  };
  ...
  const launchChatGPT = () => {
    ...
    window.open('https://chat.openai.com/?prompt=' + encodeURIComponent(enhanced.metaPrompt), '_blank');
  };
        ...
        >
          ...
          <div className="col-span-12 md:col-span-7 flex flex-col gap-6">
            ...
            <div className="relative flex-grow flex flex-col">
              ...
              <div className="relative h-full">
                <textarea
                  ...
                  style={{
                    backgroundImage: 'linear-gradient(rgba(0,0,0,0.1), rgba(0,0,0,0.1))', // Darkens the bg slightly for text contrast
                  }}
                />
                ...
                {suggestion && raw && (
                  <div className="pointer-events-none absolute inset-0 p-4 text-lg leading-relaxed">
                    ...
                    <span className="text-[var(--color-text-tertiary)]/70">{suggestion.substring(raw.length)}</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 items-center rounded-lg bg-[var(--color-bg-secondary)]/50 p-4 border border-[var(--color-glass-border)]">
              ...
              <div className="lg:col-span-2 flex flex-wrap gap-2 justify-center lg:justify-start">
                {analysis?.hasVagueTerms && <AnalysisPill icon="alert" text="Vague Terms" type="danger" />}
                {analysis?.hasActionableVerbs && <AnalysisPill icon="check" text="Actionable Verbs" type="success" />}
                {analysis?.isSpecific && <AnalysisPill icon="check" text="Specific" type="success" />}
                {analysis?.isConcise > 80 && <AnalysisPill icon="check" text="Concise" type="success" />}
                {analysis?.isConcise < 40 && <AnalysisPill icon="alert" text="Wordy" type="warning" />}
              </div>
            </div>
          ...
          <div className="col-span-12 md:col-span-5 flex flex-col gap-4 rounded-lg bg-[var(--color-bg-secondary)]/50 p-4 border border-[var(--color-glass-border)]">
            <div className="flex items-center gap-3">
              <Icon name="history" className="h-6 w-6 text-[var(--color-text-secondary)]"/>
            ...
            <div className="overflow-y-auto pr-2 space-y-3">
              {vault.length > 0 ? (
                vault.map((entry) => (
                  ...
                  >
                    <p className="truncate text-[var(--color-text-primary)] group-hover:text-white">{entry.raw}</p>
                    <div className="flex items-center gap-4 text-xs text-[var(--color-text-secondary)] mt-1">
                      ...
                      <span className="flex items-center gap-1">
                        <Icon name="check" className="h-3 w-3 text-[var(--color-glow-success)]"/>
                ...
                <div className="flex flex-col items-center justify-center h-full text-center text-[var(--color-text-secondary)]">
                    ...
                    <p>Your enhanced prompts will be saved here for future reference.</p>
                </div>
              )}
            </div>
          </div>
