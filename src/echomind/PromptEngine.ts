...
const detectMissing = (text: string): string[] => {
  ...
  if (!/\b(tone|style|format)\b/i.test(text))
    missing.push("Specify tone/style/format.");
  if (!/\b(target audience|reader|user)\b/i.test(text))
    missing.push("Define target audience.");
  return missing;
};
...
export const analyzePrompt = (raw: string): PromptAnalysis => {
  ...
  const hasActionableVerbs =
    /\b(create|generate|write|summarize|explain|translate)\b/i.test(cleaned);
  ...
  if (clarityScore < 60)
    recommendations.push("Add more detail to raise clarity.");
  ...
  return {
    ...
    wordCount,
    vagueWords: hasVagueTerms
      ? cleaned.match(/\b(vague|general|unspecific)\b/g) || []
      : [],
    missingPieces,
