...
export const DryRunProvider: FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const setMode = (newMode: DryRunMode) => {
      ...
      addToast({ type: 'info', title: 'Mode Switched', message: `Environment is now in ${newMode.toUpperCase()} mode.` });
  };
  ...
  const simulate = async <T,>(label: string, operation: () => Promise<T>, options: SimulateOptions<T> = {}): Promise<T> => {
      ...
      if (mode === 'preview') {
          if (!stub) {
              const error = new Error("Preview mode requires a 'stub' to be provided.");
              addEvent({ ...baseEventData, status: 'failure', attempts: 1, error: error.message });
              addToast({ type: 'error', title: 'Preview Error', message: error.message });
              throw error;
          }
          if (previewOutcome === 'success') {
              addEvent({ ...baseEventData, status: 'previewed', attempts: 1, result: stub.success });
              return Promise.resolve(stub.success);
          } else {
              addEvent({ ...baseEventData, status: 'previewed', attempts: 1, error: stub.failure.message });
      ...
      // --- Handle 'dry' mode ---
      if (mode === 'dry') {
          addEvent({ ...baseEventData, status: 'simulated', attempts: 1, result: 'Operation was not executed.' });
      ...
      for (let i = 0; i <= retries; i++) {
          try {
              ...
              addEvent({ ...baseEventData, status: 'success', attempts: i + 1, result });
              return result;
          } catch (error: any) {
              ...
              if (!isLastAttempt) {
                  addEvent({ ...baseEventData, status: 'retrying', attempts: i + 1, error: errorMessage });
              } else {
                  // Handle final failure
                  if (mode === 'healing') {
                      console.warn(`[HEALING-MODE] Final attempt for "${label}" failed and was suppressed. Error:`, error);
                      addEvent({ ...baseEventData, status: 'failure', attempts: i + 1, error: `HEALED: ${errorMessage}` });
                      return Promise.resolve(undefined as unknown as T); // Gracefully resolve
                  } else { // Live mode
                      addEvent({ ...baseEventData, status: 'failure', attempts: i + 1, error: errorMessage });
...
const Toast: FC<{ toast: ToastMessage; onDismiss: (id: number) => void }> = ({ toast, onDismiss }) => {
  ...
  return (
      <div className={`flex items-start w-full max-w-sm p-4 rounded-xl shadow-2xl border backdrop-blur-xl bg-gray-900/80 ${color.border} animate-toast-in`}>
          <Icon className={`w-7 h-7 mr-4 ${color.text} flex-shrink-0`} />
          <div className="flex-grow">
              <h3 className="font-bold text-white">{toast.title}</h3>
              <p className="text-sm text-gray-300">{toast.message}</p>
          </div>
          <button onClick={() => onDismiss(toast.id)} className="p-1 -m-1 text-gray-500 hover:text-white">&times;</button>
      </div>
  );
};
